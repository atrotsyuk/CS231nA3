...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        #model from equation on hw worksheet
        loss = style_weights[i] * ((gram_matrix(feats[style_layers[i]]) - style_targets[i])**2)
        style_loss += torch.sum(loss)
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    #from homework worksheet, combining equation for implementation via parts. 
    tv_loss1 = torch.sum((img[:,:,:,1:] - img[:,:,:,:-1])**2)
    tv_loss2 = torch.sum((img[:,:,1:] - img[:,:,:-1])**2)
    total_loss = tv_weight * (tv_loss1 + tv_loss2)
